...
import "../grcp/server.js";
import client from "../grcp/client.js";

const isProduction = process.env.NODE_ENV === "production";
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const PROTO_PATH = join(__dirname, "./grcp/customers.proto");
...
async function startServer() {
  ...
  app.get("*", async (req, res, next) => {
    ...
    if (!req.originalUrl.startsWith("/api")) {
      ...
      if (!httpResponse) {
        ...
        // Para fluxos HTTP, use httpResponse.pipe() em vez disso, veja https://vite-plugin-ssr.com/stream
        res.send(body);
      }
    } else {
      next();
  ...
  app.get("/api/contrato", async (req, res, next) => {
    const { page = 1, limit = 50, filter = "" } = req.query;
    client.filtrarContrato(
